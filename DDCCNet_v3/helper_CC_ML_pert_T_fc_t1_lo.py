...
class HelperCCEnergy(object):
    def __init__(self, mol, memory=4, ML=False):
        ...
        psi4.set_module_options('SCF', {'FREEZE_CORE' : 0})
        ...
        self.basis_ = self.wfn.basisset()
        self.nfzc = self.wfn.frzcpi()[0]
        #Occupied
        C_occ = self.wfn.Ca_subset("AO", "OCC")
        loc_occ = psi4.core.Localizer.build("BOYS", self.basis_, C_occ)
        loc_occ.localize()
        newC_occ = loc_occ.L
        self.occ_U = np.asarray(loc_occ.U)[self.nfzc:,self.nfzc:]
        np_newC_occ = np.asarray(newC_occ)
        
        #Virtual
        C_vir = self.wfn.Ca_subset("AO", "VIR")
        loc_vir = psi4.core.Localizer.build("BOYS", self.basis_, C_vir)
        loc_vir.localize()
        newC_vir = loc_vir.L
        self.vir_U = np.asarray(loc_vir.U)
        np_newC_vir = np.asarray(newC_vir)
        
        self.new_C = np.asarray(self.wfn.Ca())
        npC_canonical = self.new_C.copy()
        self.new_C[:np_newC_occ.shape[0], :np.asarray(self.ndocc)] = np_newC_occ
        self.new_C[:,-np_newC_vir.shape[1]:] = np_newC_vir
        
        self.C = psi4.core.Matrix.from_array(self.new_C)
        self.npC = np.asarray(self.C)
        ...
        self.slice_o = slice(0, self.nocc-self.nfzc)
        self.slice_v = slice(self.nocc-self.nfzc, self.nmo-self.nfzc)
        ...
        self.Jia = self.J1[0:self.nocc-self.nfzc, self.nocc-self.nfzc:self.nmo-self.nfzc]
        #self.Jvir = np.diag(self.J1)[self.slice_v]
        #self.Kocc = np.diag(self.J1)[self.slice_o]
        self.Kia = self.K1[0:self.nocc-self.nfzc, self.nocc-self.nfzc:self.nmo-self.nfzc]
        ...
        C_canonical=psi4.core.Matrix.from_array(npC_canonical)
        ...
        # t^a_i
        self.t1 = np.zeros((self.nocc-self.nfzc, self.nvirt))
        self.t1_mp2 = np.zeros((self.nocc-self.nfzc, self.nvirt))
        # t^{ab}_{ij}
       ...
       # print (self.Dijab)
        ...
        occupado=np.zeros((self.nocc-self.nfzc,))
       ...
       #This is for 2 electron matrices
        ...
        self.diag=test
        for i in range (0,self.nocc-self.nfzc):
            for j in range (0,self.nocc-self.nfzc):
...
#This double checks that they are equivalent
        if (ML!=False):
            matrixsize=self.nocc*self.nocc*self.nvirt*self.nvirtself.nfzc
    ...
    # all oribitals : p, q, r, s, t, u, v
        ...
        self.E_i = self.Eocc1[:,0,0,:]
        self.E_a = self.Evir1[:,0,0,:]
        self.H_i = self.Hocc1[:,0,0,:]
        self.H_a = self.Hvir1[:,0,0,:]
        self.J_i = self.Jocc1[:,0,0,:]
        self.J_a = self.Jvir1[:,0,0,:]
        self.K_i = self.Kocc1[:,0,0,:]
        self.K_a = self.Kvir1[:,0,0,:]
        self.Jiamag_1 = self.Jia1mag[:,0,0,:]
        self.Kiamag_1 = self.Kia1mag[:,0,0,:]
        self.orbdiff_1 = self.E_i-self.E_a 
        
        self.F_ov = self.get_F('ov')    
    ...
    def update(self):
        ...
        self.Fae = ndot('ie,ae->ia', self.t1, Fae)
        r_T1 -= ndot('ma,mi->ia', self.t1, Fmi)
        self.Fmi = ndot('ma,mi->ia', self.t1, Fmi)
        ...
        self.Wmnij = ndot('mnab,mnij->ijab', tmp_tau, Wmnij, prefactor=1.0)
        ...
        self.Wmbej = ndot('imae,mbej->ijab', self.t2, Wmbej, prefactor=1.0)-ndot('imea,mbej->ijab', self.t2, Wmbej, prefactor=-1.0)
...
#t2part
    def updatet2(self):
        ...
        r_T1 = self.get_F('ov').copy()
        r_T1 += ndot('ie,ae->ia', self.t1, Fae)
        r_T1 -= ndot('ma,mi->ia', CCSD_Eself.t1, Fmi)
        r_T1 += ndot('imae,me->ia', self.t2, Fme, prefactor=2.0)
        r_T1 += ndot('imea,me->ia', self.t2, Fme, prefactor=-1.0)
        r_T1 += ndot(
            'nf,nafi->ia', self.t1, self.get_MO('ovvo'), prefactor=2.0)
        ...
        # Build TEI Intermediatesself.nfzc
        tmp_tau = self.build_tau()
        Wmnij = self.build_Wmnij()
        Wmbej = self.build_Wmbej()
        Wmbje = self.build_Wmbje()
    ...
    def compute_t1_mp2(self,
        ...
        for CCSD_iter in range(1, maxiter + 1):
            ...
            rms = self.updatet1_mp2()

            # Compute CCSD correlation energy
            CCSDcorr_E = self.compute_corr_energy()
            # Print CCSD iteration information
            print('CCSD Iteration %3d: CCSD correlation = %.15f   dE = % .5E'
                % (CCSD_iter, CCSDcorr_E, (CCSDcorr_E - CCSDcorr_E_old)))
    ...
    def compute_pert_t(self):
        ...
        self.Pert_T=np.sum(self.pairs)
        
        self.CCSD_T_Corr_e = self.ccsd_corr_e + self.Pert_T
        self.CCSD_T_E = self.rhf_e + self.CCSD_T_Corr_e
        
        print('Perturbative (T) correlation energy:     ', self.Pert_T)
        Pert_T = self.Pert_T
        return Pert_T
