...
def loading_train(Train_file): 
    ...
    return Molecule_data, scaler_t2, ff_t2, scaler_t1, ff_t1, Bigfeats_combined, Bigamps_combined, Bigfeats_t1_combined, Bigamps_t1_combined, Bigints
...
def train(Molecule_array, epoch_size=100):
    ...
    for epoch in range(epoch_size): # 5 epochs at maximum
        ...
        for i, data in enumerate(Molecule_array):
            ...
            t2_pred = T2_NN(inputs_t2)
            t1_pred = T1_NN(inputs_t1)
        ...
        progressBar(epoch+1, epoch_size)

        Average_corre_error = running_corre_loss/len(Molecule_array)
        Average_rms_error = running_rms_loss/len(Molecule_array)
        Average_t2_error = running_t2_loss/len(Molecule_array)
        Average_t1_error = running_t1_loss/len(Molecule_array)
        ...
        if (epoch+1) % 1000 == 0:
            ...
            t2_tmp_model = MLP_t2_test()
            t1_tmp_model = MLP_t1_test()
            ...
            model_name = 'DDCCNetv3_CO2-mono-di-10tri_' +str(epoch+1) + '.sav'
...
Molecule_data, scaler_t2, ff_t2, scaler_t1, ff_t1, Bigfeats_combined, Bigamps_combined, Bigfeats_t1_combined, Bigamps_t1_combined, Bigints = loading_train('Train_mono-di-10tri_LA.sav')

train(Molecule_data, epoch_size=25000)
